...
while getopts :b:f:g:h:k:d:e:c:l:s:x:p:r:t:z: option
do
 case "${option}" in
 b) GITOPS_URL_BRANCH=${OPTARG};;
 f) FLUX_REPO_URL=${OPTARG};;
 g) GITOPS_SSH_URL=${OPTARG};;
 h) GITOPS_KNOWN_HOSTS_PATH=${OPTARG};;
 ...
 x) REGISTRY_SCANNING_DISABLED=${OPTARG};;
 p) PROMETHEUS_ENABLED=${OPTARG};;
...
# conditionally supply arguments to helm template command
HELM_TPL_OPTS=()
if [[ $PROMETHEUS_ENABLED == true ]]; then
  HELM_TPL_OPTS+=(--set-string annotations."prometheus\.io/port"=3030)
fi

if [[ -f "$GITOPS_KNOWN_HOSTS_PATH" ]]; then
  echo "using supplied ssh known_hosts"
  HELM_TPL_OPTS+=(--set-file ssh.known_hosts="$GITOPS_KNOWN_HOSTS_PATH")
fi
...
if ! $HELM_BIN template $RELEASE_NAME . \
        ...
        --set registry.disableScanning="$REGISTRY_SCANNING_DISABLED" \
