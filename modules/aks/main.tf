...
resource "azurerm_log_analytics_workspace" "workspace" {
  name                = "${var.cluster_name}-k8s-workspace"
...
resource "azurerm_kubernetes_cluster" "cluster" {
  ...
  addon_profile {
    ...
    # required to avoid a cluster re-create
...
resource "azurerm_management_lock" "cluster" {
  name       = "aks-cluster-lock"
  scope      = azurerm_kubernetes_cluster.cluster.id
  lock_level = "CanNotDelete"
  notes      = "Locked because all Microservices are running in this cluster"
}
